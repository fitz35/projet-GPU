...
#define INIT_TRACER(file_name, ...) time_tracer_cuda::init_tracer(file_name, __VA_ARGS__);

/**
 * @brief Start a timestamp
 * @param stamp_id The indice of the stamp in the vector of stamps
 * @param id The block to execute
 */
#define TIMESTAMP(STAMP, BLOCK) { \
time_tracer_cuda::timestamp_start(); \
BLOCK \
time_tracer_cuda::timestamp_end(STAMP); \
}

#define TIMESTAMP_ID(STAMP, ID, BLOCK) { \
time_tracer_cuda::timestamp_start(); \
BLOCK \
time_tracer_cuda::timestamp_end(STAMP, ID); \
}

#define FLUSH_TRACES(generation) time_tracer_cuda::write_traces(generation);
#define STOP_TRACER time_tracer_cuda::stop_tracer();
